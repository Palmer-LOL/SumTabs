...
function installStylesheet() {
	if (document.querySelector('link[data-sumtabs-window-actions="true"]')) return;
	const link = document.createElement("link");
	link.rel = "stylesheet";
	link.href = chrome.runtime.getURL("window-actions.css");
	link.dataset.sumtabsWindowActions = "true";
	document.head.appendChild(link);
}

function installWindowActionsUi() {
	const legacyButton = document.getElementById("closeAllInWindow");
	const section =
		legacyButton?.closest(".popup__section") ??
		document.querySelector(".popup__section--more");
...
function getUi() {
	...
	return {
		section,
		content: document.getElementById("windowActionsContent"),
		feedback: document.getElementById("popupFeedback"),
		ungroupedCount: document.getElementById("ungroupedTabCount"),
		managedCount: document.getElementById("managedTabCount"),
		unmanagedCount: document.getElementById("unmanagedTabCount"),
		pinnedCount: document.getElementById("pinnedTabCount"),
		ungroupManaged: document.getElementById("ungroupManagedGroups"),
		closeManaged: document.getElementById("closeManagedTabs"),
		closeUngrouped: document.getElementById("closeUngroupedTabs"),
		closeAllUnpinned: document.getElementById("closeAllUnpinnedTabs"),
		ungroupContext: document.getElementById("ungroupManagedContext"),
		closeAllContext: document.getElementById("closeAllUnpinnedContext"),
	};
}
...
async function closeTabsSafely(targetTabs) {
	...
	if (remainingTabs.length === 0) await chrome.tabs.create({ active: true });
...
async function ungroupManaged(ui) {
	...
	await runBusy(ui, ui.ungroupManaged, "Ungrouping managed groups…", () =>
		chrome.tabs.ungroup(context.managedTabs.map((tab) => tab.id)),
	);
	announce(
		ui,
